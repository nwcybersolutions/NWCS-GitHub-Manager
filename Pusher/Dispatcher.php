...
class Dispatcher
{
    ...
    {
        if (isset($_POST['nwcybersolutions_gm'])) {

            if ( ! current_user_can('update_plugins') || ! current_user_can('update_themes') ) {
                wp_die(__('You do not have sufficient permissions to access this page.'));
            }

            $request = $_POST['nwcybersolutions_gm'];
    ...
    {
        if ( ! isset($_GET['nwcybersolutions_gm-hook']))
            return;

        if ( ! isset($_GET['token']) || $_GET['token'] !== get_option('nwcybersolutions_gm_token')) {
