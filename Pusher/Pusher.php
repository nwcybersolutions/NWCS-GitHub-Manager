...
class Pusher implements Container
{
    ...
    public function adminMenu()
    {
        ...
        add_submenu_page('nwcybersolutions_gm', 'NWCS GitHub Manager Themes', 'Themes', 'manage_options', 'nwcybersolutions_gm-themes', array($this->make('Pusher\Dashboard'), 'getThemes'));
    }
    ...
    {
        return (bool) get_option('nwcybersolutions_gm_license_key', false);
    }
    ...
    public function registerPluginActionLinks()
    {
        ...
        $url = is_multisite()
            ? network_admin_url('admin.php?page=nwcybersolutions_gm-plugins')
            : get_admin_url(null, 'admin.php?page=nwcybersolutions_gm-plugins');
    ...
    {
        register_setting('pusher-token-settings', 'nwcybersolutions_gm_token');
        add_filter('pre_update_option_nwcybersolutions_gm_token', array($this->make('Pusher\Services\TokenGenerator'), 'refreshTokenFilter'), 10, 2);

        register_setting('pusher-license-settings', 'nwcybersolutions_gm_license_key');
        add_filter('pre_update_option_nwcybersolutions_gm_license_key', array($this->make('Pusher\License\LicenseManager'), 'activateSiteLicense'), 10, 2);
    ...
    public function nagBitbucketUsers()
    {
        ...
        if ( ! $hasBitbucketToken and $hasBitbucketUsername) {
            add_action('admin_notices', function() {
                $message = "<a href=\"admin.php?page=nwcybersolutions_gm&tab=bitbucket\">Please obtain a Bitbucket token</a> for easier and safer authentication with NWCS GitHub Manager.";
    ...
    {
        wp_register_style('nwcybersolutions_gm-styles', trailingslashit($this->pusherUrl) . 'assets/nwcybersolutions_gm.css');
        wp_enqueue_style('nwcybersolutions_gm-styles');

        wp_register_script('font_awesome', 'https://use.fontawesome.com/ad1f5276bc.js');
        wp_enqueue_script('font_awesome');

        wp_register_script('nwcybersolutions_gm-js', trailingslashit($this->pusherUrl) . 'assets/nwcybersolutions_gm.js');
        wp_enqueue_script('nwcybersolutions_gm-js');
    }
